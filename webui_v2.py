...
def get_vs_list():
    ...
    print("KB_ROOT_PATH--->:", KB_ROOT_PATH)
    # print("KB_ROOT_PATH--->:", VS_ROOT_PATH)
    if not lst:
        return lst_default
    lst.sort()
...
def get_tuning_model_list():
    lst_default = []
    if not os.path.exists(PTUNING_MODEL_PATH_ROOT):
        return lst_default
    lst = os.listdir(PTUNING_MODEL_PATH_ROOT)
    _lst = []
    for tuning_model in lst:
        tmps = os.listdir(PTUNING_MODEL_PATH_ROOT + tuning_model)
        for tmp in tmps:
            if "checkpoint-" in tmp:
                _lst.append(PTUNING_MODEL_PATH_ROOT + tuning_model + "/" + tmp)
    if not _lst:
        return lst_default
    _lst.sort()
    return lst_default + _lst
...
def reinit_model(llm_model, embedding_model, llm_history_len, no_remote_model, use_ptuning_v2, select_tuning_model, use_lora, top_k,
                 history):
    try:
        if use_ptuning_v2:
            fine_tuning_model_path = select_tuning_model
...
def get_vector_store(vs_id, files, sentence_size, history, one_conent, one_content_segmentation):
    ...
    # xaw
    # vs_path = os.path.join(KB_ROOT_PATH, vs_id)
...
def change_vs_name_input(vs_id, history):
    if vs_id == "新建知识库":
        ...
        vs_path = os.path.join(KB_ROOT_PATH, vs_id, "vector_store")
        # vs_path = os.path.join(UPLOAD_ROOT_PATH, vs_id)
...
def reinit_vector_store(vs_id, history):
    try:
        ...
        # vs_path, loaded_files = local_doc_qa.init_knowledge_vector_store(os.path.join(UPLOAD_ROOT_PATH, vs_id),
...
init_message = f"""欢迎使用 浪潮通信信息 Web UI！
...
with gr.Blocks(css=block_css, theme=gr.themes.Default(**default_theme_args)) as demo:
    ...
    with gr.Tab("模型配置"):
        ...
        use_ptuning_v2 = gr.Checkbox(USE_PTUNING_V2,
                                     ...
                                     interactive=True)
        select_tuning_model = gr.Dropdown(get_tuning_model_list(),
                                          label="请选择要加载的知识库",
                                          interactive=True,
                                          value=get_tuning_model_list()[0] if len(get_tuning_model_list()) > 0 else None
                                          )
        ...
        load_model_button.click(reinit_model, show_progress=True,
                                inputs=[llm_model, embedding_model, llm_history_len, no_remote_model, use_ptuning_v2, select_tuning_model,
