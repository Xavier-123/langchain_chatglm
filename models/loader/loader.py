...
from configs.model_config import LLM_DEVICE, PTUNING_MODEL_PATH_ROOT, PTUNING_MODEL_PATH2, PTUNING_DIR
...
class LoaderCheckPoint:
    ...
    def __init__(self, params: dict = None):
        ...
        # xaw
        self.tuning_model = params.get("tuning_model", PTUNING_DIR)
        ...
        if not any([self.llm_device.lower() == "cpu",
            ...
            if torch.cuda.is_available() and self.llm_device.lower().startswith("cuda"):
                ...
                if num_gpus < 2 and self.device_map is None:
                    ...
                    # 加入微调模型
                    if self.tuning_model:
                        # print("load tuning_model......")
                        print("load tuning_model......" + self.tuning_model)
                        # print("load tuning_model......" + PTUNING_MODEL_PATH2)
                        prefix_state_dict = torch.load(
                                # os.path.join(PTUNING_MODEL_PATH2,
                                os.path.join(self.tuning_model,
                                "pytorch_model.bin"))
                    ...
                    # # 加入微调模型
                    # prefix_state_dict = torch.load(
                    #         os.path.join(PTUNING_MODEL_PATH,
                    #         "pytorch_model.bin"))
                    # new_prefix_state_dict = {}
                    # for k, v in prefix_state_dict.items():
                    #     if k.startswith("transformer.prefix_encoder."):
    ...
    def chatglm_auto_configure_device_map(self, num_gpus: int) -> Dict[str, int]:
        ...
        if 'chatglm2' in self.model_name:
            device_map = {
                ...
                f"{layer_prefix}.prefix_encoder.embedding.weight": 0,
    ...
    def reload_model(self):
        ...
        # if self.ptuning_dir:
        #     try:
        #         print(f'{self.ptuning_dir}/config.json')
        #         # print(f'{self.tuning_model}/config.json')
        #         prefix_encoder_file = open(Path(f'{self.ptuning_dir}/config.json'), 'r')
        #         # prefix_encoder_file = open(Path(f'{self.tuning_model}/config.json'), 'r')
