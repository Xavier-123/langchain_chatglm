...
def is_exist_groupfile(group, filename):
    url = f"http://{solr_url}:{solr_port}/solr/is_exist_groupfile?group={group}&filename={filename}"
...
async def Inspur_knowledge_id_list():
    ...
    if len(local_knowledge_id_list) > 0:
        ...
        for knowledge_id in all_knowledge_id_names:
            url = f"http://{solr_url}:{solr_port}/solr/is_exist_group?group={knowledge_id}"
            test_post_response = requests.post(url)
            res = json.loads(test_post_response.content.decode('utf-8'))
            if res["result"] == True:
...
async def Inspur_list_docs(
    ...
    url = f"http://{solr_url}:{solr_port}/solr/is_exist_group?group={knowledge_base_id}"
    test_post_response = requests.post(url)
    res = json.loads(test_post_response.content.decode('utf-8'))
...
async def Inspur_delete_docs(
    ...
    if res.code == 200:
        ...
        url = f"http://{solr_url}:{solr_port}/solr/is_exist_groupfile?group={knowledge_base_id}&filename={filename}"
        test_post_response = requests.post(url)
        res = json.loads(test_post_response.content.decode('utf-8'))
        if res["result"] == False:
        ...
        url = f"http://{solr_url}:{solr_port}/solr/delete_group_title?group={knowledge_base_id}&ttitle={filename}"
        test_post_response = requests.post(url)
        res = json.loads(test_post_response.content.decode('utf-8'))
...
async def Inspur_delete_Knowledge(
    ...
    # 6、删除solr中的group
    solr_del_url = f"http://{solr_url}:{solr_port}/solr/delete_group?group={knowledge_base_id}"
