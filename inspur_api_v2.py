...
from configs.model_config import (VS_ROOT_PATH, UPLOAD_ROOT_PATH, EMBEDDING_DEVICE,
                                  EMBEDDING_MODEL, NLTK_DATA_PATH, USE_PTUNING_V2, PTUNING_DIR, PTUNING_MODEL_PATH_ROOT,
...
# # 是否使用微调
# use_ptuning_v2 = os.environ.get("USE_PTUNING_V2")
# ptuning_dir = os.environ.get("PTUNING_DIR")
...
'''新增动态加载model'''
@app.post("/Inspur/reload_model")
def reload_model(model_name: str = "chatglm2-6b", fine_tuning_model: str = None):

    # 不加载微调模型
    use_tuning_model = True
    if fine_tuning_model is None:
        use_tuning_model = False

    fine_tuning_model_path = None

    if fine_tuning_model:
        fine_tuning_model_path = PTUNING_MODEL_PATH_ROOT + "/" + fine_tuning_model

    llm_model = model_name
    no_remote_model = False
    llm_model_ins = shared.loaderLLM_tuning(llm_model=llm_model, no_remote_model=no_remote_model, fine_tuning_model_path=fine_tuning_model_path)
    # llm_model_ins = shared.loaderLLM_tuning(llm_model, no_remote_model, use_ptuning_v2, fine_tuning_model_path)
...
'''获取微调模型名称'''
@app.get("/Inspur/get_tuning_model")
async def get_tuning_model():
    tuning_model_list = os.listdir(PTUNING_MODEL_PATH_ROOT)
    res = []
    for model in tuning_model_list:
        # checkpoint_exit = False
        checkpoint_list = os.listdir(PTUNING_MODEL_PATH_ROOT + "/" + model)
...
def api_start(args_dict):
    # global app
    global local_doc_qa
    global use_tuning_model
    use_tuning_model = USE_PTUNING_V2

    host, port = args_dict["host"], args_dict["port"]
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--use_ptuning_v2", type=bool, default=USE_PTUNING_V2)
    parser.add_argument("--ptuning_dir", type=str, default=PTUNING_DIR)
